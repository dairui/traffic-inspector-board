...
void port_select_init(void)
{
    ...
    //LED7   USART_IAP×´Ì¬ÏÔÊ¾
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_15;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;     
    GPIO_Init(GPIOA, &GPIO_InitStructure);  
    
    //LED8   INTERNET_IAP×´Ì¬ÏÔÊ¾
...
void TIM2_configuration(u8 haf_second)
{
    ...
    time_init.TIM_CounterMode = TIM_CounterMode_Up;         //??¨ºy¡¤??¨°
    time_init.TIM_Period = haf_second * 200;               //??¨ºy?¨¹?¨²
    time_init.TIM_Prescaler = 35999;                        //?¡è¡¤??¦Ì,????¨º¡ÀPSC=N+1
    time_init.TIM_ClockDivision = TIM_CKD_DIV4;             //2¨¦?¨´?¨¹?¨²
